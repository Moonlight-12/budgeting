...
export async function GET(request: NextRequest) {
  try {
    ...
    let response = await fetch(`${backendUrl}/api/v1/transactions/monthly?utcOffset=${utcOffset}`, {
      headers: {
    ...
    if (response.status === 403 && refreshToken) {
      ...
      if (refreshResponse.ok) {
        ...
        if (newAccessToken) {
          response = await fetch(`${backendUrl}/api/v1/transactions/monthly?utcOffset=${utcOffset}`, {
            headers: {
