...
export async function PUT(request: NextRequest) {
  try {
    ...
    let response = await fetch(`${backendUrl}/api/v1/budget/savings/allocate`, {
      method: "PUT",
    ...
    if (response.status === 403 && refreshToken) {
      ...
      if (refreshResponse.ok) {
        ...
        if (newAccessToken) {
          response = await fetch(`${backendUrl}/api/v1/budget/savings/allocate`, {
            method: "PUT",
    ...
    console.error("Allocate PUT error:", error);
